...
resource "cloudflare_workers_script" "scheduled_teardown" {
  ...
  plain_text_binding {
    ...
    text = "50 20 * * *"  # Notification at 20:45 UTC (21:45 CET)
  }

  plain_text_binding {
    name = "TEARDOWN_CRON"
    text = "5 21 * * *"  # Teardown at 21:00 UTC (22:00 CET)
  }
...
resource "cloudflare_workers_cron_trigger" "scheduled_teardown" {
  ...
  schedules   = [
    "50 20 * * *",  # Notification at 20:45 UTC (21:45 CET)
    "5 21 * * *",   # Teardown at 21:00 UTC (22:00 CET)
  ]
}
...
resource "cloudflare_pages_domain" "control_plane" {
  ...
  domain       = "control-raul-kiener.nona.company"
...
resource "cloudflare_zero_trust_access_application" "control_plane" {
  ...
  cors_headers {
    allowed_origins   = ["https://control-raul-kiener.nona.company"]
